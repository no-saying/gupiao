...
### 2.3 标签分析：为什么旧标签不是"泄露"

**旧标签**: `open[t+4]/open[t] - 1`（T 日开盘买入 → T+4 开盘卖出）
**新标签**: `(open[t+5]-open[t+1])/open[t+1]`（T+1 开盘买入 → T+5 开盘卖出）

**实证结果**（60 天回测）：

| 指标 | 旧标签 | 新标签 | 结论 |
|:-----|:------|:------|:-----|
| 日间排名自相关 | 0.7226 | 0.7209 | 几乎相同 |
| 隔夜跳空可预测性 | 0.30 | — | 不是纯噪声 |
| 时间窗口重叠 | 75% | — | T+1→T+4 三天重叠 |

**为什么旧标签效果更好**：

1. **目标定义更清晰**：旧标签问"给定今天数据，未来 4 天怎么走"，模型能学。新标签多了一个要求"先猜明天开盘价"，而隔夜跳空受美股、政策等日线特征无法捕捉的因素影响。

2. **75% 时间重叠**：旧标签覆盖 T+1→T+4（3 天），新标签覆盖 T+1→T+5（4 天）。模型学到的大部分趋势信号（75%）在真实交易中依然有效。

3. **14 周回测 Sharpe 1.53 是真金**：严格样本外测试，模型没见过测试周数据。不是泄露虚高。

**结论**：旧标签不是数据泄露，而是**不让模型承担它预测不了的任务（隔夜跳空）**。
...
**只测一周。** 这意味着：
- 一个 Sharpe 2.0 的模型可能在测试周亏钱（方差）
- 一个 Sharpe 0.5 的模型可能在测试周夺冠（运气）
- 长期平均收益高 ≠ 单周好
...
| 指标 | 阈值 | 说明 |
|:-----|:----|:-----|
| 单周最差收益 | > -2% | 任何一周亏超 2% 说明模型有致命漏洞 |
| 14 周正收益周数 | ≥ 12/14 | 胜率低于 85% 的模型不可信 |
| 空仓检测 | 不强制满仓 | 市场弱时硬凑 5 只会系统性拉低底线 |
...
| 指标 | 含义 | 判断方法 |
|:-----|:-----|:---------|
| **模型共识度** | 12 个子模型对选股是否一致 | 选出的股票在 ≥70% 子模型中排进 top-20 |
| **行业集中度** | 5 只股票的行业分布 | 同行业不超过 2 只 |
| **候选池质量** | 门控通过数 | ≥5 通过 = 市场有明确机会；<3 通过 = 市场混沌 |
...
```
模型 A: [+2%, +3%, +1%, +2%, +1%, +3%, +2%, -15%]  → Mean=-0.1%, 但有一周暴亏
模型 B: [+2%, +1%, +0%, +1%, +2%, +1%, +1%, +2%]     → Mean=1.25%, 从未亏钱
```

模型 A 均值差但偶尔爆发，模型 B 均值好但平庸。**单周赛制下，模型 B 远优于模型 A**——因为 A 有 12.5% 概率直接出局。

**判断方法**：看收益的 5% 分位数（最差 5% 的情况），而不是均值。最小值的绝对值应该 < 均值的 50%。

#### 2.4.4 训练时 vs 推理时的指标对应

| 训练指标 | 对应推理表现 | 阈值 |
|:---------|:-----------|:-----|
| Pair Accuracy | 排序能力 | > 0.52 算有信号 |
| Val RankIC | 截面排序质量 | > 0.03 可用 |
| 模型间 Spearman 相关 | 集成多样性 | 0.7-0.85 最佳（太高=冗余，太低=噪声） |
| Val Loss 趋势 | 是否过拟合 | train/val loss 同向下降=正常，背离=过拟合 |
