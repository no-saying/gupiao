...
class TimeEncoder(nn.Module):
    ...
    def __init__(self, n_features: int, d_model: int = D_MODEL,
        ...
        # 双向会把 hidden_dim 翻倍，所以我们后面需要投影回来
        self.gru = nn.GRU(
            d_model, d_model,
            num_layers=n_layers,
            batch_first=True,
        ...
        # 输出投影：双向 GRU 的 hidden state 是 d_model*2，需要投影回 d_model
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: (B*N, T, F)  — 所有股票的时序数据沿 batch 维堆叠
               这里 B 是样本数，N 是股票数，T 是时间步，F 是特征数
               把 B*N 看作 batch 维度，让 GRU 并行处理所有股票的编码

        Returns:
            (B*N, d_model)  — 每只股票一个 embedding 向量
        """
        x = self.input_proj(x)                           # (BN, T, F) → (BN, T, d_model)

        # GRU 输出：
        #   output: 每个时间步的隐态 (不需要，我们只用最后一步)
        #   h_n:    最后一层的隐态 (D*layers, BN, d_model)
        #           D=2（双向），所以第一维是 2*layers
        _, h_n = self.gru(x)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None
        ...
        Returns:
            ...
            attn_weights: (B, N, N)         — 注意力权重矩阵
                           ...
                           可用于事后分析股票间的关联关系
        """
        # key_padding_mask=True 的位置会被屏蔽（注意力权重设为 -inf）
        # 这样无效股票不会影响有效股票的计算
        key_padding = (mask == 0) if mask is not None else None

        # Self-Attention: Q=K=V=x（自注意力）
...
class PortfolioPredictor(nn.Module):
    ...
    def __init__(self, n_features: int,
        ...
        # 堆叠多层 Transformer
        # 第 1 层学到简单的两两关联（如"工商银行和建设银行相关"）
        # 第 2 层学到更复杂的间接关联（如"银行股和保险股通过金融板块关联"）
        self.transformers = nn.ModuleList([
            CrossSectionalTransformer() for _ in range(n_transformer_layers)
        ])

        # 评分头：将 d_model 维的股票表示映射为一个标量分数
        # d_model → d_model/2 → 1，中间加 GELU 激活和 Dropout 防止过拟合
